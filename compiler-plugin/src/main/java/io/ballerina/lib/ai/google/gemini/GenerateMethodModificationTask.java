 ...
 * for types used in Gemini model provider's `generate` method calls.
 *
 * @since 1.0.0
 */
class GenerateMethodModificationTask implements ModifierTask<SourceModifierContext> {
    ...
    public GenerateMethodModificationTask(AiGeminiCodeModifier.AnalysisData analysisData) {
    ...
    public void modify(SourceModifierContext modifierContext) {
        ...
        for (ModuleId moduleId : currentPackage.moduleIds()) {
            ...
            Optional<Symbol> geminiModelProviderSymbol =
                    types.getTypeByName(GEMINI_MODEL_PROVIDER_MODULE_ORG, GEMINI_MODEL_PROVIDER_MODULE_NAME,
                            GEMINI_MODEL_PROVIDER_MODULE_VERSION, GEMINI_MODEL_PROVIDER_NAME);

            for (DocumentId documentId : documentIds) {
                analyzeDocument(module, documentId, semanticModel, geminiModelProviderSymbol);
            }
    ...
    private void analyzeDocument(Module module, DocumentId documentId, SemanticModel semanticModel,
        ...
        analyzeGenerateMethod(semanticModel, modulePartNode, geminiModelProviderSymbol, this.analysisData);
    }
    ...
    private void analyzeGenerateMethod(SemanticModel semanticModel,
                                       ModulePartNode modulePartNode, Optional<Symbol> geminiModelProviderSymbol,
                                       AiGeminiCodeModifier.AnalysisData analysisData) {
        new GenerateMethodJsonSchemaGenerator(semanticModel, geminiModelProviderSymbol, analysisData)
    ...
    private class GenerateMethodJsonSchemaGenerator extends NodeVisitor {
        ...
        private final ClassSymbol geminiProviderSymbol;

        public GenerateMethodJsonSchemaGenerator(SemanticModel semanticModel,
                                                 Optional<Symbol> geminiModelProviderSymbolOpt,
                                                 AiGeminiCodeModifier.AnalysisData analyserData) {
            this.semanticModel = semanticModel;
            this.typeMapper = analyserData.typeMapper;
            if (geminiModelProviderSymbolOpt.isEmpty()) {
                this.geminiProviderSymbol = null;
                return;
            }

            Symbol geminiModelProviderSymbol = geminiModelProviderSymbolOpt.get();
            if (geminiModelProviderSymbol instanceof ClassSymbol geminiModelProviderClassSymbol) {
                this.geminiProviderSymbol = geminiModelProviderClassSymbol;
            } else {
                this.geminiProviderSymbol = null;
            }
        }
        ...
        void generate(ModulePartNode modulePartNode) {
            if (this.geminiProviderSymbol == null) {
                return;
            }
        ...
        public void visit(RemoteMethodCallActionNode remoteMethodCallActionNode) {
            ...
            semanticModel.typeOf(expression).ifPresent(expressionTypeSymbol -> {
                if (expressionTypeSymbol.subtypeOf(this.geminiProviderSymbol)) {
